 ...
 * finite mixture of unordered categories and a Beta distribution.
 *
 ...
 * to the population prior parameters.
 *
...
transformed parameters {
  // Zeros and Ones
  vector[N_sumcat] theta_logit = mu_obs_logit
    + gamma_raw_logit[jj_cat] + delta_raw_logit[kk_cat];

  // Beta distribution with logit link
  vector[N_con] mu_beta_logit = inv_logit(mu_obs_logit
...
generated quantities {
  ...
  vector[J] gamma_err = inv_logit(gamma_raw_logit);
  vector[K] delta_err = inv_logit(delta_raw_logit);
  ...
  vector[J] epsilon_gamma = inv_logit(gamma_raw_logit)
    - inv_logit(rep_vector(mu_j_logit, J));
  vector[K] epsilon_delta = inv_logit(delta_raw_logit)
    - inv_logit(rep_vector(mu_k_logit, K));
  vector[N_sumcat] epsilon_y_cat = to_vector(y_cat) - inv_logit(theta_logit);
  ...
  real y_rep[N];
  {
    ...
    vector[K] delta_raw_logit_rep
      = to_vector(normal_rng(rep_vector(mu_k_logit, K), sigma_delta));
    vector[N] theta_logit_rep = mu_obs_logit
      + gamma_raw_logit_rep[jj] + delta_raw_logit_rep[kk];
    vector[N] mu_beta_logit_rep = inv_logit(mu_obs_logit
      + gamma_raw_logit_rep[jj] + delta_raw_logit_rep[kk]);
    ...
    for (n in 1:N) {
      if (bernoulli_rng(lambda)) {
        y_rep[n] = bernoulli_logit_rng(theta_logit_rep[n]);
      } else {
        y_rep[n] = beta_rng(shape_beta_rep[1][n], shape_beta_rep[2][n]);
      }
    }
  }
  ...
  // Superpopulation and finite-population standard deviations
  {
    ...
    vector[2] sigma2_mixcomp = [ mu_mixcomp[1] * (1 - mu_mixcomp[1]),
    ...
    vector[N_sumcat] e_y_cat = to_vector(y_cat) - inv_logit(theta_logit);
